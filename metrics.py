...
def classification_result(y_true, y_probs, model_name, saved_path):
    ...
    # --- All-task AUROC ---
    all_auroc_scores = {}
    auroc_log = ["\n\n--- All-Task AUROC Scores ---"]

    fig_all, ax_all = plt.subplots(figsize=(12, 9))
    for task in CHEXPERT_CLASSES:
        ...
        try:
            ...
            all_auroc_scores[task] = score
            auroc_log.append(f"{task:>35}: {score:.4f}")
            fpr, tpr, _ = roc_curve(task_true, task_probs)
            ax_all.plot(fpr, tpr, lw=1.5, label=f"{task} (AUC={score:.3f})")
        except ValueError:
            warning_msg = f"{task:>35}: N/A (No positive samples)"
            print(warning_msg)
            auroc_log.append(warning_msg)

    if all_auroc_scores:
        mean_all_auroc = np.mean(list(all_auroc_scores.values()))
        auroc_log.append("-" * 50)
        auroc_log.append(f"{'Mean All-Task AUROC':>35}: {mean_all_auroc:.4f}")
        print(f"\nMean All-Task AUROC: {mean_all_auroc:.4f}")

    ax_all.plot([0, 1], [0, 1], color='gray', linestyle='--', label="Random Guess")
    ax_all.set_xlim([0.0, 1.0])
    ax_all.set_ylim([0.0, 1.05])
    ax_all.set_xlabel('False Positive Rate (1 - Specificity)', fontsize=12)
    ax_all.set_ylabel('True Positive Rate (Sensitivity)', fontsize=12)
    ax_all.set_title(f'ROC Curves for All Tasks: {model_name}', fontsize=14)
    ax_all.legend(loc="lower right", fontsize=8)
    ax_all.grid(alpha=0.3)
    plt.savefig(saved_path / f"{model_name}_roc_curves_all_tasks.png", bbox_inches='tight')
    plt.close()

    # --- Stanford Competition Tasks AUROC ---
    competition_auroc_scores = []
    ...
    for task in COMPETITION_TASKS:
        ...
        try:
            score = roc_auc_score(task_true, task_probs)
            competition_auroc_scores.append(score)
    ...
    if competition_auroc_scores:
        mean_auroc = np.mean(competition_auroc_scores)
        auroc_log.append("-" * 30)
        auroc_log.append(f"{'Mean Competition AUROC':>20}: {mean_auroc:.4f}")
        print(f"Mean Competition AUROC: {mean_auroc:.4f}\n")
